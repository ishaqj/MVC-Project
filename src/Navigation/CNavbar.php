...
class CNavbar
{
    ...
    public function create()
    {
        ...
        $websiteName = isset($menu['websiteName']) ? "<div class='container'>
    <div class='navbar-header'>
      <span class='navbar-brand'> {$menu['websiteName']}</span>
    </div>" : null;
        $userName = isset($menu['userName']) ? "<p class='navbar-text navbar-right'>Signed in as <b>{$menu['userName']}</b></p>" : null;
        $wrapper = $menu['wrapper'];

        return "\n<{$wrapper}{$id}{$class}>{$websiteName}{$userName}{$html}</{$wrapper}>\n";
    }
}
